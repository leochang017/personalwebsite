...
PROJECTS:
1. LLM Microgrid Agents (ACTIVE, Apr 2026-Present) — Research with Prof. Yongfeng Zhang (Rutgers CS, direct collaboration, not WISE Lab). Investigating whether populations of LLM agents (one per household) can negotiate peer-to-peer to allocate scarce energy during grid outages with fairness, robustness to bad information, and auditable explanations. Phase 1 (complete): a 30-household discrete-time simulator with realistic battery physics (rate limits, round-trip efficiency, depth-of-discharge floor), running on real NREL ResStock 15-min load profiles for 30 Texas homes and real NREL NSRDB Austin solar irradiance. 70 tests, mypy strict, GitHub Actions CI on every push. First real-data baseline on a 12-hour overnight outage: a round-robin sharing strategy saves 13.4 kWh of residential load and reduces welfare-inequality Gini coefficient by 65.8% vs. no coordination. Phase 2 (next): LLM-agent layer with memory, reflection, and natural-language inter-agent negotiation, drawing on Park et al. (arXiv:2304.03442) as a reference rather than a fork. Phase 3-4: benchmark suite, interactive web demo, paper targeting ICLR Tackling Climate Change with ML, NeurIPS Computational Sustainability, or AAMAS applied track. Repo: github.com/leochang017/microgrid-llm-coordination. Tech: Python, NumPy, NREL ResStock, NREL NSRDB, Anthropic API (Phase 2), multi-agent systems.

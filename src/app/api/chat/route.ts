...
STATS: 4 Projects, 8 Work Positions across 4 Countries (US, China, Malaysia, South Korea), 5 Leadership Roles, 19+ Awards (15 Competitions, 16 Top Placements), 580+ Work Hours, 660+ Volunteer Hours.
...
4. Phase Spector — Lead Game Developer & Designer (Dec 2025-Present) [ACTIVE]. Tagline: "Rewind. Strike. Survive." Status: PLAYABLE (released). Top-down wave-based arcade shooter with unique time-rewind combat mechanic. Built in Godot 4.6/GDScript. Core mechanic: record up to 1.5s of movement path, then rewind at 2x speed to damage enemies along trail (rewind attack does 100 dmg, one-shotting basic enemies) and deflect incoming projectiles back at attackers (deflected projectiles do 50 dmg). Controls: Arrow Keys to move, Space to rewind. Rewind pipeline: Move → Record path → Rewind at 2x → All enemies freeze in place → Player projection ghost sprites mark the retraced trail → Attack along path & deflect projectiles → Cooldown before next use. Player: 100 HP, 3 lives, 1.5s i-frames after damage and 1.0s after a rewind. 3 enemy types: Melee Chargers (50 HP, 8-direction pursuit, 10 score), Ranged Shooters (50 HP, strafe + projectile fire, 25 score), Mini-Bosses (300 HP, 100 score, every 5th wave). Mini-bosses cycle three attack patterns: 5-projectile spread shot (60° spread), 3 aimed shots, and a telegraphed melee dive that speeds up each wave. Killing a mini-boss drops either a Powerup (extends rewind buffer length in increments of max/8, growing the rewind UI bar) or a Healing Pickup (restores a life). Both are pause-aware so they don't get lost mid-rewind. Score multiplier: chain kills within a 6-second window build a multiplier from 1.0x up to 2.0x in 0.2x increments; multiplier bar shifts through five colors as it climbs and pauses while rewinding. Taking damage resets the chain. Wave system: starts at 2 enemies, caps at 7 per wave, base enemy speed climbs each wave up to 350. Persistent top-5 named high score leaderboard, shown on game-over screen. Polished game flow: custom title screen, walkthrough instruction screen with on-screen labels for each control, hit-flash sprites, deflect/hurt/rewind/powerup sound effects, parallax scrolling background that pauses with the rest of the world during rewind. Tech: wave-based spawning, layered Area2D collision system, event-driven signal architecture, dynamic scene instancing, position history ring buffer (1.5s), invincibility frame management, group-based pause system. Available for 500+ PDS students. Playable in browser.
...
Focus Areas: Machine Learning, AI, LLM Agents, Multi-Agent Systems, Quantum Computing, Industrial AI, Smart Materials, Web Dev, Data Science, Game Dev.

WHAT DRIVES LEO: Building projects & applications (NapkinNotes 80+ users 170+ notes, Phase Spector for 500+ PDS students, LLM microgrid agents research at Rutgers), Research & discovery (JEI publication, Rutgers LLM-agents work with Prof. Yongfeng Zhang), Cross-disciplinary curiosity (summer 2026 work spans LLM multi-agent systems for microgrids at Rutgers, AI for continuous-variable quantum key distribution at Zhongke Guoguang Quantum in Beijing, industrial ML on lift telemetry at Nippon Lift in Penang, and smart-materials/seismic research with Prof. Eunsoo Choi at Hongik in Seoul), Global perspective (summer 2026 internships across Beijing, Penang, and Seoul; 6+ years directing education for children at Ti-Ratana in Kuala Lumpur), Community impact (660+ volunteer hours, $8,000 fundraised, weekly lessons in Malaysia), Competitive spirit (fencing since age 6, USA Dance National DanceSport Champion, United States Dance Championships Pro Am Finalist, programming competitions, competitive writing including PYAA Gold Award and 2x Scholastic Silver Keys, Science Olympiad).
